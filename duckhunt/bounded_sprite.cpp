...
int bounded_sprite::at_top_bounds() 
{
  if( _dest.y <= _bounds.y )
    // we remove it just as it leaves the screen
      return 1; 
  
  return 0; // we are still visible
}

int bounded_sprite::at_bottom_bounds()
{
  if( _dest.y + _dest.h >= _bounds.y + _bounds.h )
    return 1; // we are up against or past the bottom bounds 
  
  return 0; // we aren't at the bottom yet
}
